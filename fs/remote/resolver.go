...
func (r *Resolver) resolveFetcher(ctx context.Context, fc *fetcherConfig) (f fetcher, size int64, err error) {
	...
	if fc.desc.Size == 0 {
		...
		if err != nil {
			return nil, 0, fmt.Errorf("%s from %s: %w", ErrFailedToRetrieveLayerSize, socihttp.RedactHTTPQueryValuesFromString(hf.realURL), err)
		}
	}
...
func newHTTPFetcher(ctx context.Context, fc *fetcherConfig) (*httpFetcher, error) {
	...
	for _, host := range fc.hosts {
		...
		if authClient, ok := tr.(*socihttp.AuthClient); ok {
			...
			globalRetryClient := authClient.Client()
			// If the Blob specific HTTP configurations are different
			// than the ones present in the global HTTP config, we will
			// need to create a new retryable client.
			if globalRetryClient.RetryMax != fc.maxRetries ||
				globalRetryClient.RetryWaitMin != fc.minWait ||
				globalRetryClient.RetryWaitMax != fc.maxWait ||
				globalRetryClient.HTTPClient.Timeout != fc.fetchTimeout {

				// Get the inner concrete HTTP client.
				globalStandardClient := globalRetryClient.HTTPClient
				if globalTransport, ok := globalStandardClient.Transport.(*http.Transport); ok {
					newRetryClient := resolver.CloneRetryableClient(globalRetryClient)
