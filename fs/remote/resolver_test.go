...
func TestRetry(t *testing.T) {
	...
	rclient.Backoff = resolver.BackoffStrategy
	f := &httpFetcher{
		realBlobURL: "test",
		client:      MockClient(&rhttp.RoundTripper{Client: rclient}),
	}
...
func TestCustomUserAgent(t *testing.T) {
	...
	rt := &userAgentRoundTripper{expectedUserAgent: resolver.UserAgent}
	retryClient.HTTPClient = MockClient(rt)

	ac, _ := socihttp.NewAuthClient(MockAuthHandler(), socihttp.WithRetryableClient(retryClient), socihttp.WithHeader(resolver.GlobalHeaders()))
	f := &httpFetcher{
		realBlobURL: "dummyregistry",
		client:      MockClient(ac),
	}
	regions := []region{{b: 0, e: 1}}
	_, err := f.fetch(context.Background(), regions, true)
	if err != nil {
