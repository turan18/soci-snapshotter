...
func TestNetworkRetry(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "5s network interruption, no retries allowed, failure",
			config: retryConfig{
				maxRetries:         -1,
				minWaitMsec:        0,
				maxWaitMsec:        0,
...
`
			...
			cmdStdoutCh := make(chan []byte)
			cmdStderrCh := make(chan []byte)
			cmdReadErrorCh := make(chan error)
			go func() {
				doneCh := make(chan struct{})
				outData := make([]byte, 1)
				errData := make([]byte, 1)
				go func() {
					_, err := cmdStdout.Read(outData)
					if err != nil {
						cmdReadErrorCh <- err
						doneCh <- struct{}{}
					}
				...
				// Wait until we either fail to Read or get some data from stdout or stderr
				<-doneCh
			}()
			...
			select {
			case err := <-cmdReadErrorCh:
				t.Fatalf("failed to read stdout/stderr: %v", err)
			case data := <-cmdStderrCh:
				if tt.config.expectedSuccess {
					t.Fatalf("expected Read request to succeed; got data in stderr: %s", string(data))
				}
			case data := <-cmdStdoutCh:
				if !tt.config.expectedSuccess {
					t.Fatalf("expected Read request to fail; got data in stdout: %s", string(data))
				}
