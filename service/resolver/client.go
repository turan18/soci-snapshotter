...
func newAuthClient(retryClient *rhttp.Client, creds func(string) (string, string, error)) (*socihttp.AuthClient, error) {
...
// newRefSpecWithHost takes a reference spec return a new one with the host portion replaced by `host`
...
func newRefSpecWithHost(imgRefSpec reference.Spec, host string) (reference.Spec, error) {
	...
	mirrorImgRefSpec, err := reference.Parse(fmt.Sprintf("%s/%s%s", host, path, tag))
	if err != nil {
		return reference.Spec{}, err
	}
	return mirrorImgRefSpec, nil
}
