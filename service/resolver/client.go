...
// TODO: Should any function here but NewGlobalClient be exported?
...
func newGlobalClient(config config.RetryableHTTPClientConfig, creds Credential) (*http.Client, error) {
	...
	retryClient := newRetryableClientFromConfig(config)
	...
	authClientOpts := []socihttp.AuthClientOpt{socihttp.WithRetryableClient(retryClient),
...
// newRetryableClientFromConfig creates a go http.Client which will automatically
